PROGRAM cdftransportiz
  ...
  CHARACTER(LEN=80) :: cfilet ,cfileout='section_htrp.dat', &
       &                       cfileouts='section_strp.dat.dat',&
       &                       cfileoutv='section_vtrp.dat', &
  ...
  narg= iargc()
  IF ( narg < 2  ) THEN
     PRINT *,' Usage : cdftransportiz [-test  u v ]   gridUfile gridVfile   ''limit of level'' '
 ...
 DO jclass=1,nclass
     ...
     DO jseg = 1, nn-1
        ...
        ELSE IF ( ryy(jseg) == ryy(jseg+1) ) THEN
           ...
           saltrp(jseg)=ztrpvs(i0+ist,j0,jclass)*norm_v
        ELSE
           WRITE(numout,*)' ERROR :',  rxx(jseg),ryy(jseg),rxx(jseg+1),ryy(jseg+1)
        END IF
        voltrpsum = voltrpsum+voltrp(jseg)
        heatrpsum = heatrpsum+heatrp(jseg)
        saltrpsum = saltrpsum+saltrp(jseg)
     END DO   ! next segment
     IF (jclass == 1 )  WRITE(numout,*)  '% nada LONmin LATmin LONmax LATmax'
     IF (jclass == 1 )  WRITE(numout,*)  '% Top(m)  Bottom(m)  MassTrans(Sv) HeatTrans(PW) SaltTrans(kt/s)'
     IF (jclass == 1 )  WRITE(numout,*) 0 ,imin, imax, jmin, jmax
     IF (jclass == 1 )  WRITE(numout,9003) 0 ,gla(1),gphi(1), gla(nn-1), gphi(nn-1)
