PROGRAM cdf_remake_bathy 
  ...
  !!                     ***  PROGRAM  cdf_remake_bathy  ***
  !!=====================================================================
  !!  ** Purpose : Recompute bathymetry from e3t vertical intrgration
  !!
  ...
  INTEGER(KIND=4), PARAMETER                   :: pnvarout = 3             ! number of output variables
  INTEGER(KIND=4)                              :: ncout                    ! ncid of output file
  INTEGER(KIND=4), DIMENSION(pnvarout)         :: id_varout                ! id of output variables
  ...
  REAL(wp), DIMENSION(:,:)    , ALLOCATABLE    :: nav_lon, nav_lat         ! t-grid horizontal
  ...
  narg= iargc()
  IF ( narg == 0 ) THEN
     PRINT *,' usage : cdf_remake_bathy -mz ZGR-file -msk MASK-file -o OUT-file'
     PRINT *,'      '
     PRINT *,'     PURPOSE :'
     PRINT *,'      Remake bathymetry from vertical integration of e3t.'
     PRINT *,'      '
     PRINT *,'     ARGUMENTS :'
     PRINT *,'       -mh HGR-file       : netcdf file for vertical mesh'
...
CONTAINS
   ...
   SUBROUTINE CreateOutput
    ...
    ierr    = putheadervar(ncout , cf_mz   ,  npi, npj, 1, nav_lon, nav_lat                   )

    dtim = getvar1d(cf_mz, cn_vtimec,   npt     )
