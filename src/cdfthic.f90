PROGRAM cdfthic
  ...
  narg= iargc()
  IF ( narg == 0 ) THEN
     PRINT *,' usage : cdfthic [-vvl <file>] [-T] [-U] [-V]'
     PRINT *,'                 [-ssh <file>]'
     PRINT *,'                 [-full]'
     PRINT *,'                 [-T] [-U] [-V]'
     PRINT *,'                 [-o OUT-file]'
     ...
     PRINT *,'      '
     PRINT *,'     OPTIONS :'
     PRINT *,'        (default)        : partial step computation (constant grid), on T points'
     PRINT *,'        -full            : full step computation (constant grid)' 
  ...
  DO jt=1,npt
     ...
     ierr = putvar(ncout, id_varout(jvar), REAL(dl_vint1), jk, 1, 1, ktime=jt)
